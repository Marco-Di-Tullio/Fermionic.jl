...
@testset "Fermionic.jl" begin
    ...
    @test ssp(State_sparse_fixed(sparse([1,6],[1,1],[1/sqrt(2),im/sqrt(2)]),Op(4),2)) == 1.0
    ...
    @test rhomnd(State_sparse_complex(1/sqrt(2)*(im*cdm(Op(6),1)*cdm(Op(6),2)*cdm(Op(6),3)*cdm(Op(6),4)+cdm(Op(6),3)*cdm(Op(6),4)*cdm(Op(6),5)*cdm(Op(6),6))*vacuum(Op(6)),Op(6)),2)[4,4] == 0.5
    ...
    @test round(trp(State_complex([0,0,0,im/sqrt(2),0,0,0,0,0,0,0,0,1/sqrt(2),0,0,0],Op(4)),[1,4])[2,2],digits=5) == -0.5
    @test round(trp(State_sparse_complex(1/sqrt(2)*(im*cdm(Op(4),1)*cdm(Op(4),2)+cdm(Op(4),3)*cdm(Op(4),4))*vacuum(Op(4)),Op(4)),[2,3])[2,2],digits=5) == 0.5
end
